//! End-to-end tests for the `bl` binary (skills marketplace + bl-specific
...
use common::{
    bl_command, calculate_tool_schema, list_tools_response, output_text, temp_test_dir,
    write_bl_org_config, write_extensions_catalog, MockResponse, MockServer,
    BL_TOOLS_CALL_TOOL_PATH, BL_TOOLS_LIST_TOOLS_PATH,
};
...
fn marketplace_skill_summary() -> Value {
    json!({
        ...
        "name": "BuilderLab Tools",
        "description": "Use BuilderLab CLI tool wrappers from agent workflows.",
        "status": "stable",
        "visibility": "builtin",
        "enabled": true,
        "latest_version_id": "ver_builtin_builderlab_tools_0_1_0",
        "latest_content_sha256": "content-sha",
        "source_id": "src_builtin_builderlab",
        "source_revision": "builtin:builderlab-tools:0.1.0",
        "source_path": "builtin-skills/builderlab-tools",
        "tags": ["builderlab", "tools"],
        "teams": ["builderlab"],
...
/// One bundle (`starter-pack`) that contains `builderlab-tools`, for tests
...
fn agent_state(bl_home: &Path, slug: &str) -> PathBuf {
    bl_home
        .join("agents")
        .join("installed")
...
fn write_managed_agent(bl_home: &Path, home: &Path, slug: &str, document: &[u8]) {
    let target = agent_target(home, slug);
    let state = agent_state(bl_home, slug);
...
#[test]
fn bl_agents_are_discoverable_and_install_requires_a_slug_without_network() {
    let output = bl_command().arg("--help").output().expect("run bl help");
    let (stdout, stderr) = output_text(&output);
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl describe-commands");
    ...
    let server = MockServer::start(vec![]);
    let bl_home = temp_test_dir("bl-agents-install-missing-slug");
    write_bl_org_config(&bl_home, "test");
    let mut before = fs::read_dir(&bl_home)
        .expect("read bl home before parsing")
        .map(|entry| entry.expect("read bl home entry").file_name())
        .collect::<Vec<_>>();
    before.sort();
    let output = bl_command()
        .env("BL_HOME", &bl_home)
        ...
        .args(["agents", "install"])
        .output()
        .expect("run bl agents install without slug");
    let requests = server.finish();
    ...
    assert!(requests.is_empty(), "requests were: {requests:#?}");
    let mut after = fs::read_dir(&bl_home)
        .expect("read bl home after parsing")
        .map(|entry| entry.expect("read bl home entry").file_name())
        .collect::<Vec<_>>();
    after.sort();
    assert_eq!(
        after, before,
        "missing-slug parsing must not mutate BL_HOME"
    );
    fs::remove_dir_all(bl_home).expect("remove bl home");
}

#[test]
fn bl_agents_update_requires_a_managed_install() {
    let sandbox = temp_test_dir("bl-agents-update-absent");
    let bl_home = sandbox.join("bl-home");
    ...
    assert!(!agent_state(&bl_home, "release-notes").exists());

    fs::remove_dir_all(sandbox).expect("remove absent update sandbox");
}

#[test]
fn bl_agents_use_agent_routes_and_stable_catalog_output() {
...
#[test]
fn bl_agents_lifecycle_is_idempotent_and_local_queries_stay_offline() {
    let sandbox = temp_test_dir("bl-agents-lifecycle");
    let bl_home = sandbox.join("bl-home");
    let home = sandbox.join("home");
    write_bl_org_config(&bl_home, "test");
    ...
    let run = |arguments: &[&str]| {
        bl_command()
            ...
            .args(arguments)
            .output()
            .expect("run bl agents lifecycle command")
    };
    ...
    let run_offline = |arguments: &[&str]| {
        bl_command()
            ...
            .args(arguments)
            .output()
            .expect("run local bl agents command")
    };
...
#[test]
fn bl_agents_report_local_ownership_conflicts_without_modifying_user_content() {
    let sandbox = temp_test_dir("bl-agents-conflicts");
    let bl_home = sandbox.join("bl-home");
    let home = sandbox.join("home");
    write_bl_org_config(&bl_home, "test");
    let document = agent_document("Release Notes", "Managed content.");

    write_managed_agent(&bl_home, &home, "alpha", document.as_bytes());
    write_managed_agent(&bl_home, &home, "bravo", document.as_bytes());
    fs::remove_file(agent_target(&home, "bravo")).expect("remove managed target for missing case");

    let malformed_state = agent_state(&bl_home, "invalid!");
    ...
    write_managed_agent(&bl_home, &home, "changed", document.as_bytes());
    ...
    let mismatched_state = agent_state(&bl_home, "mismatched");
    ...
    let directory_state = agent_state(&bl_home, "directory");
    ...
    let symlink_target = {
        ...
        let state = agent_state(&bl_home, "symlink");
        fs::create_dir_all(state.parent().expect("state parent")).expect("create state parent");
        fs::write(
            &state,
...
#[test]
fn bl_agents_preserve_managed_pairs_for_failure_envelopes() {
    let sandbox = temp_test_dir("bl-agents-failure-envelopes");
    let bl_home = sandbox.join("bl-home");
    let home = sandbox.join("home");
    let document = agent_document("Release Notes", "Managed content.");
    write_bl_org_config(&bl_home, "test");
    write_managed_agent(&bl_home, &home, "release-notes", document.as_bytes());
    let target = agent_target(&home, "release-notes");
    let state = agent_state(&bl_home, "release-notes");
    ...
    let run = |server: &MockServer, arguments: &[&str]| {
        bl_command()
            ...
            .args(arguments)
            .output()
            .expect("run bl agents failure command")
    };
...
fn marketplace_install_plan(zip_bytes: &[u8], artifact_sha: &str, artifact_size: usize) -> Value {
    json!({
        "plan_id": "plan_phase1_builderlab_tools",
        ...
        "operations": [{
            ...
            "artifact": {
                "id": "art_builderlab_tools",
                "download_url": "/v1/marketplace/artifacts/art_builderlab_tools/download",
                "sha256": artifact_sha,
    ...
    MockResponse::bytes(
        ...
        serde_json::to_vec(&json!({
            "error": {
                ...
                "retryable": false,
                "details": [{
                    "path": "skills/builderlab-tools/SKILL.md",
                    "field": "description",
...
/// as if a previous `bl skills install` had completed.
...
#[test]
fn bl_root_help_lists_apps_skills_and_tools() {
    let output = bl_command().arg("--help").output().expect("run bl help");
    let (stdout, stderr) = output_text(&output);
    ...
    assert!(stdout.contains("BuilderLab command line tools"));
    assert!(!stdout.contains("--local-dev"));
}

#[test]
fn bl_root_description_lists_apps_not_appkit() {
    let output = bl_command()
        .arg("--describe-commands")
        .output()
        .expect("describe bl commands");
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_tools_root_help_does_not_require_org() {
    let temp = temp_test_dir("bl-tools-help-no-org");
    let bl_home = temp.join("bl-home");
...
#[test]
fn bl_completions_emit_shell_script() {
    let output = bl_command()
        .args(["completions", "bash"])
        .output()
        .expect("run bl completions");
    let (stdout, stderr) = output_text(&output);

    assert!(output.status.success(), "stderr was: {stderr}");
    assert!(stdout.contains("bl"), "stdout was: {stdout}");
    assert!(
        stdout.contains("bl,agents)"),
    ...
    for subcommand in [
        ...
        assert!(
            stdout.contains(&format!("bl__subcmd__agents,{subcommand})")),
...
#[test]
fn bl_skills_list_fetches_marketplace_skills_and_bundle_membership() {
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl skills list");
    let requests = server.finish();
    let (stdout, stderr) = output_text(&output);

    assert!(output.status.success(), "stderr was: {stderr}");
    let response = serde_json::from_str::<Value>(&stdout).expect("parse list output");
    assert_eq!(response["items"][0]["slug"], json!("builderlab-tools"));
...
#[test]
fn bl_skills_list_uses_custom_kgoose_service_path() {
    let server = MockServer::start(vec![skill_page_response(), starter_pack_bundles_response()]);
...
#[test]
fn bl_skills_list_formats_marketplace_skills_for_humans() {
    let server = MockServer::start(vec![skill_page_response(), starter_pack_bundles_response()]);
    ...
    assert!(
        stdout.contains("  builderlab-tools [stable]"),
        "stdout was: {stdout}"
    );
    assert!(
        stdout.contains("    Use BuilderLab CLI tool wrappers from agent workflows."),
        "stdout was: {stdout}"
    );
    assert!(
        stdout.contains("    name: BuilderLab Tools"),
        "stdout was: {stdout}"
    );
    assert!(
        stdout.contains("    tags: builderlab, tools"),
        "stdout was: {stdout}"
    );
    assert!(
    ...
    assert!(
        stdout.contains("Install one with: bl skills install <slug>"),
        "stdout was: {stdout}"
    );
}

#[test]
fn bl_skills_list_groups_installed_and_available_skills() {
    let temp = temp_test_dir("bl-skills-list-grouped");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let skills_home = temp.join("skills-home");
    write_installed_package(&skills_home, "builderlab-tools", "content-sha", &["agents"]);
    let server = MockServer::start(vec![
        MockResponse::json(json!({
            "items": [
    ...
    let output = bl_command()
        .env("BL_HOME", &bl_home)
        .env("BL_SKILLS_HOME", &skills_home)
        .env("BL_SKILLS_PACKAGES_DIR", skills_home.join("packages"))
        .env("KGOOSE_BASE_URL", &server.base_url)
        .args(["skills", "list"])
        .output()
        .expect("run bl skills list");
    ...
    assert!(
        stdout.contains("  builderlab-tools [stable] (up to date)"),
        "stdout was: {stdout}"
    );
    assert!(
        stdout.contains("    version: ver_builtin_builderlab_tools_0_1_0"),
        "stdout was: {stdout}"
    );
    assert!(
...
#[test]
fn bl_skills_search_passes_query_filter() {
...
#[test]
fn bl_skills_show_prints_skill_detail() {
    let server = MockServer::start(vec![skill_detail_response()]);

    let output = bl_command()
        .env("KGOOSE_BASE_URL", &server.base_url)
        .args(["skills", "show", "builderlab-tools", "--json"])
        .output()
        .expect("run bl skills show");
    let requests = server.finish();
    let (stdout, stderr) = output_text(&output);

    assert!(output.status.success(), "stderr was: {stderr}");
    let response = serde_json::from_str::<Value>(&stdout).expect("parse show output");
    assert_eq!(response["slug"], json!("builderlab-tools"));
    assert_eq!(
        response["latest_version_id"],
        json!("ver_builtin_builderlab_tools_0_1_0")
    );
    assert_eq!(requests.len(), 1);
    assert_eq!(
        requests[0].path,
        "/api/goose/v1/marketplace/skills/builderlab-tools"
    );
}

#[test]
fn bl_skills_bundles_lists_bundles() {
    let server = MockServer::start(vec![MockResponse::json(json!({
        "items": [{
            "slug": "starter-pack",
            "name": "Starter Pack",
...
#[test]
fn bl_skills_ignores_legacy_profile_server_url_and_auth() {
    let server = MockServer::start(vec![skill_page_response(), empty_bundles_response()]);
    let temp = temp_test_dir("bl-skills-profile-config");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    fs::create_dir_all(&bl_home).expect("create bl home");
    fs::write(
        bl_home.join("skills.yaml"),
        format!(
...
#[test]
fn bl_skills_list_uses_stored_session_credential_despite_legacy_profile_auth() {
    let server = MockServer::start(vec![skill_page_response(), empty_bundles_response()]);
    let temp = temp_test_dir("bl-skills-list-session");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
...
#[test]
fn bl_local_dev_discovers_checked_in_config_from_ancestor() {
    let server = MockServer::start(vec![MockResponse::json(json!({
        "target_registry": {
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl local dev doctor");
    let requests = server.finish();
    let (stdout, stderr) = output_text(&output);
    ...
    assert!(response["bl_skills_home"]
        .as_str()
        .expect("bl_skills_home string")
        .ends_with(".bl/local-dev/skills"));
    assert_eq!(requests.len(), 1);
    assert_eq!(requests[0].method, "GET");
    assert_eq!(
        requests[0].path,
...
#[test]
fn bl_skills_env_playpen_adds_baggage_header() {
    let server = MockServer::start(vec![skill_page_response(), empty_bundles_response()]);

    let output = bl_command()
        .env("BL_KGOOSE_PLAYPEN", "baxen")
        .env("KGOOSE_BASE_URL", &server.base_url)
        .args(["skills", "list", "--json"])
        .output()
        .expect("run bl skills list");
...
#[test]
fn bl_auth_status_without_token_is_local_and_unauthenticated() {
    let temp = temp_test_dir("bl-auth-status");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
...
#[test]
fn bl_auth_status_requires_org_in_json_mode() {
    let temp = temp_test_dir("bl-auth-status-missing-org");
    let bl_home = temp.join("bl-home");
    fs::create_dir_all(&bl_home).expect("create bl home");
    ...
    assert!(
        payload["error"]["message"]
            ...
            .contains("bl config set org <org>"),
        "stderr was: {stderr}"
    );
    fs::remove_dir_all(temp).expect("remove temp dir");
}
...
#[test]
fn bl_auth_status_uses_org_routed_custom_base_url() {
    let temp = temp_test_dir("bl-auth-status-org-base");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
...
#[test]
fn bl_auth_status_uses_auth_me_for_stored_file_session() {
    let temp = temp_test_dir("bl-auth-status-stored");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let storage_path = temp.join("auth-sessions.json");
    let server = MockServer::start(vec![MockResponse::json(json!({
...
#[test]
fn bl_auth_status_errors_never_echo_a_reflected_session() {
    let secret = "reflected_session_credential_123456";
    let server = MockServer::start(vec![
        MockResponse::text(500, secret),
        MockResponse::text(500, secret),
    ]);
    let temp = temp_test_dir("bl-auth-status-redaction");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
    ...
    for args in [vec!["auth", "status"], vec!["auth", "status", "--json"]] {
        let output = bl_command()
            ...
            .args(args)
            .output()
            .expect("run failing bl auth status");
        let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_auth_login_uses_valid_stored_file_session() {
    let temp = temp_test_dir("bl-auth-login-stored");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let storage_path = temp.join("auth-sessions.json");
    let server = MockServer::start(vec![MockResponse::json(json!({
...
#[test]
fn bl_auth_login_env_playpen_adds_baggage_to_stored_session_check() {
...
#[test]
fn bl_auth_logout_removes_stored_file_session() {
    let temp = temp_test_dir("bl-auth-logout");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let storage_path = temp.join("auth-sessions.json");
    let server = MockServer::start(vec![MockResponse::json(json!({}))]);
    ...
    let output = bl_command()
        ...
        .args(["auth", "logout", "--json"])
        .output()
        .expect("run bl auth logout again");
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_workspace_list_prints_accessible_workspaces_as_json() {
    let temp = temp_test_dir("bl-workspace-list");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
    let server = MockServer::start(vec![MockResponse::json(json!({
...
#[test]
fn bl_workspace_switch_flag_skips_list_and_stores_rotated_credential() {
    let temp = temp_test_dir("bl-workspace-switch");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
    let server = MockServer::start(vec![MockResponse::json(json!({
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl workspace switch");
    let requests = server.finish();
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_workspace_switch_current_workspace_keeps_stored_credential() {
    let temp = temp_test_dir("bl-workspace-switch-current");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
    let server = MockServer::start(vec![MockResponse::json(json!({
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl workspace switch current");
    let _requests = server.finish();
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_workspace_switch_persists_rotated_credential_before_validating_workspace() {
    let temp = temp_test_dir("bl-workspace-switch-missing-workspace");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
    let server = MockServer::start(vec![MockResponse::json(json!({
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl workspace switch with malformed response");
    let _requests = server.finish();
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_workspace_switch_rejects_invalid_rotated_credential_before_storage() {
    let temp = temp_test_dir("bl-workspace-switch-invalid-credential");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
    let server = MockServer::start(vec![MockResponse::json(json!({
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl workspace switch with invalid credential");
    let _requests = server.finish();
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_workspace_errors_escape_untrusted_server_text() {
    let temp = temp_test_dir("bl-workspace-error-safety");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
    ...
    let output = bl_command()
        ...
        .args(["workspace", "list", "--json"])
        .output()
        .expect("run forbidden bl workspace list");
    let _requests = server.finish();
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_workspace_switch_requires_flag_outside_a_tty() {
    let temp = temp_test_dir("bl-workspace-switch-noninteractive");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
    write_browser_auth_session(
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run non-interactive bl workspace switch");
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_config_set_and_get_roundtrip() {
    let temp = temp_test_dir("bl-config-prefs");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let skills_home = temp.join("skills-home");

    let set_org = bl_command()
        .env("BL_HOME", &bl_home)
        .env("BL_SKILLS_HOME", &skills_home)
        .env("BL_SKILLS_PACKAGES_DIR", skills_home.join("packages"))
        .args(["config", "set", "org", " Test-Org ", "--json"])
        .output()
        .expect("run bl config set org");
    let (_set_org_stdout, set_org_stderr) = output_text(&set_org);
    assert!(set_org.status.success(), "stderr was: {set_org_stderr}");

    let get_org = bl_command()
        .env("BL_HOME", &bl_home)
        .env("BL_SKILLS_HOME", &skills_home)
        .env("BL_SKILLS_PACKAGES_DIR", skills_home.join("packages"))
        .args(["config", "get", "org", "--json"])
        .output()
        .expect("run bl config get org");
...
#[test]
fn bl_config_set_org_repairs_invalid_existing_org() {
    let temp = temp_test_dir("bl-config-repair-org");
    let bl_home = temp.join("bl-home");
    fs::create_dir_all(&bl_home).expect("create bl home");
    fs::write(bl_home.join("config.yaml"), "org: bad_org\n").expect("write invalid config");
    ...
    let saved = fs::read_to_string(bl_home.join("config.yaml")).expect("read repaired config");
    assert!(saved.contains("org: test-org"), "saved config was: {saved}");
    fs::remove_dir_all(temp).expect("remove temp dir");
}

#[test]
fn bl_skills_auth_is_removed_but_config_alias_still_works() {
    let temp = temp_test_dir("bl-skills-alias");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let skills_home = temp.join("skills-home");

    let status = bl_command()
        .env("BL_HOME", &bl_home)
        .args(["skills", "auth", "status", "--json"])
        .output()
        .expect("run bl skills auth status");
    ...
    let get = bl_command()
        ...
        .output()
        .expect("run bl skills config get");
...
#[test]
fn bl_skills_list_surfaces_marketplace_error_envelope() {
    let server = MockServer::start(vec![marketplace_error_response(
        404,
        "skill_not_found",
        "Skill was not found.",
    ...
    assert!(
        stderr.contains(
            "details: skills/builderlab-tools/SKILL.md.description: description is required"
        ),
        "stderr was: {stderr}"
    );
}
...
#[test]
fn bl_skills_list_json_errors_are_structured() {
    let server = MockServer::start(vec![marketplace_error_response(
        404,
        "skill_not_found",
        "Skill was not found.",
...
#[test]
fn bl_skills_install_downloads_verifies_and_installs_into_isolated_home() {
    let zip_bytes = skill_zip(&[
        ("SKILL.md", "# BuilderLab Tools\n"),
        ("SETUP.md", "No setup.\n"),
    ]);
    let artifact_sha = sha256_hex(&zip_bytes);
    let temp = temp_test_dir("bl-skills-install");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let skills_home = temp.join("skills-home");
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl skills install");
    let requests = server.finish();
    let (stdout, stderr) = output_text(&output);

    assert!(output.status.success(), "stderr was: {stderr}");
    let response = serde_json::from_str::<Value>(&stdout).expect("parse install output");
    assert_eq!(response["installed"][0]["slug"], json!("builderlab-tools"));
    assert_eq!(
    ...
    assert_eq!(metadata["source_id"], json!("src_builtin_builderlab"));

    // Link into the registry-provided agents directory.
    let link = agents_dir.join("builderlab-tools");
    assert!(
    ...
    assert_eq!(
        requests[3].path,
        "/api/goose/v1/marketplace/artifacts/art_builderlab_tools/download"
    );
    fs::remove_dir_all(temp).expect("remove temp dir");
}

#[test]
fn bl_skills_install_rejects_unsafe_plan_before_artifact_fetch_or_root_escape() {
    let temp = temp_test_dir("bl-skills-unsafe-plan-slug");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let skills_home = temp.join("skills-home");
    let agents_dir = temp.join("agents-skills");
...
#[test]
fn bl_skills_install_and_update_restore_package_when_target_linking_fails() {
    for action in ["install", "update"] {
        let zip_bytes = skill_zip(&[("SKILL.md", "# New BuilderLab Tools\n")]);
        let artifact_sha = sha256_hex(&zip_bytes);
        let temp = temp_test_dir(&format!("bl-skills-{action}-target-recovery"));
        let bl_home = temp.join("bl-home");
        let skills_home = temp.join("skills-home");
        let invalid_target = temp.join("target-is-a-file");
        write_bl_org_config(&bl_home, "test");
        write_installed_package(&skills_home, "builderlab-tools", "old-content", &["claude"]);
        ...
        assert_eq!(
            fs::read_to_string(skills_home.join("packages/builderlab-tools/SKILL.md"))
...
#[test]
fn bl_skills_install_canonical_agents_dir_holds_real_package() {
    let zip_bytes = skill_zip(&[("SKILL.md", "# BuilderLab Tools\n")]);
    let artifact_sha = sha256_hex(&zip_bytes);
    let temp = temp_test_dir("bl-skills-install-canonical");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let skills_home = temp.join("skills-home");
    let agents_dir = temp.join("agents-skills");
    ...
    let output = bl_command()
        ...
        .env("BL_SKILLS_PACKAGES_DIR", &agents_dir)
        .env("KGOOSE_BASE_URL", &server.base_url)
        .args([
            "skills",
            "install",
    ...
    let output = bl_command()
        ...
        .env("BL_SKILLS_PACKAGES_DIR", &agents_dir)
        .env("KGOOSE_BASE_URL", "http://127.0.0.1:9")
        .args(["skills", "remove", "builderlab-tools", "--yes", "--json"])
        .output()
        .expect("run bl skills remove");
    let (stdout, stderr) = output_text(&output);
    assert!(output.status.success(), "stderr was: {stderr}");
...
#[test]
fn bl_skills_install_surfaces_install_plan_error_envelope() {
    let temp = temp_test_dir("bl-skills-install-plan-error");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let agents_dir = temp.join("agents-skills");
    let server = MockServer::start(vec![
    ...
    let output = bl_command()
        .env("BL_HOME", &bl_home)
        .env("BL_SKILLS_HOME", temp.join("skills-home"))
        .env("BL_SKILLS_PACKAGES_DIR", temp.join("skills-home/packages"))
        .env("KGOOSE_BASE_URL", &server.base_url)
        .args([
            "skills",
            "install",
...
#[test]
fn bl_skills_install_surfaces_artifact_error_envelope() {
    let zip_bytes = skill_zip(&[("SKILL.md", "# BuilderLab Tools\n")]);
    let artifact_sha = sha256_hex(&zip_bytes);
    let temp = temp_test_dir("bl-skills-artifact-error");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let agents_dir = temp.join("agents-skills");
    let server = MockServer::start(vec![
    ...
    assert_eq!(requests.len(), 4);
    assert!(!temp.join("skills-home/packages/builderlab-tools").exists());
    fs::remove_dir_all(temp).expect("remove temp dir");
}

#[test]
fn bl_skills_install_refuses_checksum_mismatch() {
    ...
    let temp = temp_test_dir("bl-skills-checksum");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let agents_dir = temp.join("agents-skills");
    let server = MockServer::start(vec![
...
#[test]
fn bl_skills_install_refuses_unsafe_zip_paths() {
    let zip_bytes = skill_zip(&[
        ("SKILL.md", "# BuilderLab Tools\n"),
        ("../escape.md", "nope\n"),
    ]);
    let artifact_sha = sha256_hex(&zip_bytes);
    let temp = temp_test_dir("bl-skills-path-safety");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let agents_dir = temp.join("agents-skills");
    let server = MockServer::start(vec![
...
#[test]
fn bl_skills_install_backs_up_unmanaged_package_before_replacing_it() {
    let zip_bytes = skill_zip(&[("SKILL.md", "# BuilderLab Tools\n")]);
    let artifact_sha = sha256_hex(&zip_bytes);
    let temp = temp_test_dir("bl-skills-unmanaged");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let agents_dir = temp.join("agents-skills");
    let server = MockServer::start(vec![
    ...
    let unmanaged = temp.join("skills-home/packages/builderlab-tools");
    ...
    let backups = fs::read_dir(&backup_root)
        ...
        .filter(|path| {
            path.file_name()
                ...
                .is_some_and(|name| name.starts_with("builderlab-tools-"))
        })
        .collect::<Vec<_>>();
...
#[test]
fn bl_skills_install_human_output_reports_real_folder_backup() {
    let temp = temp_test_dir("bl-skills-backup-output");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let skills_home = temp.join("skills-home");
    let packages_dir = skills_home.join("packages");
    let agents_dir = temp.join("agents-skills");
    let source = temp.join("source/builderlab-tools");
    ...
    let existing = packages_dir.join("builderlab-tools");
    ...
    let output = bl_command()
        .env("BL_HOME", &bl_home)
        .env("BL_SKILLS_HOME", &skills_home)
        .env("BL_SKILLS_PACKAGES_DIR", &packages_dir)
        .env("KGOOSE_BASE_URL", &server.base_url)
        .args([
            "skills",
            "install",
...
#[test]
fn bl_skills_install_rejects_unresolvable_version_pin() {
    let zip_bytes = skill_zip(&[("SKILL.md", "# BuilderLab Tools\n")]);
    let artifact_sha = sha256_hex(&zip_bytes);
    let temp = temp_test_dir("bl-skills-version-pin");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let agents_dir = temp.join("agents-skills");
    let server = MockServer::start(vec![
...
#[test]
fn bl_skills_install_local_path_installs_without_marketplace() {
    let temp = temp_test_dir("bl-skills-local-path");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let agents_dir = temp.join("agents-skills");
    let server = MockServer::start(vec![capabilities_response(&agents_dir)]);
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl skills install local path");
    let requests = server.finish();
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_skills_update_reports_up_to_date_skills() {
    let temp = temp_test_dir("bl-skills-update-noop");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let skills_home = temp.join("skills-home");
    let agents_dir = temp.join("agents-skills");
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl skills update");
    let requests = server.finish();
    let (stdout, stderr) = output_text(&output);

    assert!(output.status.success(), "stderr was: {stderr}");
    let response = serde_json::from_str::<Value>(&stdout).expect("parse update output");
    assert_eq!(response["up_to_date"], json!(["builderlab-tools"]));
...
#[test]
fn bl_skills_installed_reports_update_availability() {
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl skills installed");
    let requests = server.finish();
    let (stdout, stderr) = output_text(&output);
...
#[cfg(unix)]
#[test]
fn bl_skills_which_reports_link_state() {
    let temp = temp_test_dir("bl-skills-which");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let skills_home = temp.join("skills-home");
    let agents_dir = temp.join("agents-skills");
    ...
    std::os::unix::fs::symlink(
        skills_home.join("packages/builderlab-tools"),
        agents_dir.join("builderlab-tools"),
    )
    ...
    let output = bl_command()
        ...
        .env("BL_SKILLS_PACKAGES_DIR", skills_home.join("packages"))
        .env("KGOOSE_BASE_URL", "http://127.0.0.1:9")
        .args(["skills", "which", "builderlab-tools", "--json"])
        .output()
        .expect("run bl skills which");
    let (stdout, stderr) = output_text(&output);
...
#[cfg(unix)]
#[test]
fn bl_skills_remove_deletes_links_and_package() {
    let temp = temp_test_dir("bl-skills-remove");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    let skills_home = temp.join("skills-home");
    let agents_dir = temp.join("agents-skills");
    ...
    let response = serde_json::from_str::<Value>(&stdout).expect("parse remove output");
    assert_eq!(response["removed_package"], json!(true));
    assert!(!skills_home.join("packages/builderlab-tools").exists());
    assert!(
        fs::symlink_metadata(agents_dir.join("builderlab-tools")).is_err(),
...
#[test]
fn bl_skills_doctor_offline_reports_server_failure() {
    let temp = temp_test_dir("bl-skills-doctor-offline");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
...
#[test]
fn bl_shipped_artifact_contains_no_apps_test_transport() {
    let binary = fs::read(env!("CARGO_BIN_EXE_bl")).expect("read shipped bl test artifact");
    for forbidden in [
        "BL_APPS_E2E_CONTROL_PLANE_URL",
        "BL_APPS_E2E_AUTH_URL",
        "BL_APPS_E2E_CREDENTIAL",
        "BL_APPS_E2E_RESOLVE_ADDR",
        ...
        assert!(
            ...
            "shipped bl artifact contained Apps test-only material {forbidden:?}"
        );
    }
}

#[test]
fn bl_apps_help_distinguishes_external_and_internal_paths() {
    let output = bl_command()
        .args(["apps", "--help"])
        .output()
        .expect("run bl apps help");
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_apps_inspection_help_exposes_filters_and_app_ids() {
    let list = bl_command()
        .args(["apps", "list", "--help"])
        .output()
        .expect("run bl apps list help");
    ...
    for subcommand in ["get", "versions"] {
        let output = bl_command()
            ...
            .output()
            .unwrap_or_else(|error| panic!("run bl apps {subcommand} help: {error}"));
        let (stdout, stderr) = output_text(&output);
        assert!(output.status.success(), "stderr was: {stderr}");
        for expected in [
            "<APP_ID>",
...
#[test]
fn bl_apps_ready_and_debug_help_expose_their_arguments() {
    let ready = bl_command()
        .args(["apps", "ready", "--help"])
        .output()
        .expect("run bl apps ready help");
    ...
    let debug = bl_command()
        .args(["apps", "debug", "--help"])
        .output()
        .expect("run bl apps debug help");
...
#[test]
fn bl_apps_rollback_help_exposes_optional_target_and_environment() {
    let output = bl_command()
        .args(["apps", "rollback", "--help"])
        .output()
        .expect("run bl apps rollback help");
    let (stdout, stderr) = output_text(&output);
...
fn bl_apps_delete_requires_confirmation_before_auth_or_network() {
    let output = bl_command()
        ...
        .output()
        .expect("run bl apps delete without confirmation");
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_apps_delete_requires_an_explicit_environment() {
    let output = bl_command()
        .args([
            "apps",
            "delete",
        ...
        .output()
        .expect("run bl apps delete without environment");
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_apps_access_help_exposes_get_and_full_policy_replacement() {
    let access_output = bl_command()
        .args(["apps", "access", "--help"])
        .output()
        .expect("run bl apps access help");
    ...
    let get_output = bl_command()
        .args(["apps", "access", "get", "--help"])
        .output()
        .expect("run bl apps access get help");
    ...
    let set_output = bl_command()
        .args(["apps", "access", "set", "--help"])
        .output()
        .expect("run bl apps access set help");
...
fn bl_apps_access_set_requires_explicit_restricted_viewer_clearing_before_auth_or_network() {
    let output = bl_command()
        ...
        .output()
        .expect("run bl apps access set without a viewer or clearing confirmation");
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_apps_access_set_requires_visibility_before_auth_or_network() {
    let output = bl_command()
        .args([
            "apps",
            "access",
        ...
        .output()
        .expect("run bl apps access set without visibility");
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_apps_ready_requires_version_before_auth_or_network() {
    let output = bl_command()
        .args([
            "apps",
            "ready",
        ...
        .output()
        .expect("run bl apps ready without version id");
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_apps_contract_rejects_loopback_before_reading_or_sending_the_session() {
    let kgoose = MockServer::start(vec![]);
    let control_plane = MockServer::start(vec![]);
    let temp = temp_test_dir("bl-apps-loopback-origin");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("auth-sessions.json");
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl apps contract with loopback origin");
...
#[test]
fn bl_apps_contract_rejects_arbitrary_https_origin() {
    let temp = temp_test_dir("bl-apps-arbitrary-origin");
    let bl_home = temp.join("bl-home");
    write_bl_org_config(&bl_home, "test");
    ...
    let output = bl_command()
        ...
        .output()
        .expect("run bl apps contract with arbitrary origin");
    let (stdout, stderr) = output_text(&output);
...
#[test]
fn bl_apps_json_without_a_session_exits_promptly_with_auth_required() {
    let temp = temp_test_dir("bl-apps-json-auth-required");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("missing-auth-sessions.json");
    write_bl_org_config(&bl_home, "test");

    let mut child = bl_command()
        .env("BL_HOME", &bl_home)
        .env("BL_AUTH_STORAGE", "file")
        .env("BL_AUTH_STORAGE_FILE", &storage_path)
        .args([
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("start noninteractive bl apps contract");
    let deadline = Instant::now() + Duration::from_secs(10);
    let status = loop {
        if let Some(status) = child.try_wait().expect("poll bl apps contract") {
            break status;
        }
        if Instant::now() >= deadline {
            child.kill().expect("stop hung bl apps contract");
            child.wait().expect("reap hung bl apps contract");
            panic!("bl apps contract did not fail promptly without a session");
        }
...
#[test]
fn bl_apps_pipeline_without_a_session_never_starts_browser_login() {
    let temp = temp_test_dir("bl-apps-pipeline-auth-required");
    let bl_home = temp.join("bl-home");
    let storage_path = temp.join("missing-auth-sessions.json");
    write_bl_org_config(&bl_home, "test");
    ...
    let mut child = bl_command()
        ...
        .stderr(Stdio::piped())
        .spawn()
        .expect("start piped bl apps contract");
    let deadline = Instant::now() + Duration::from_secs(10);
    let status = loop {
        if let Some(status) = child.try_wait().expect("poll bl apps contract") {
            break status;
        ...
        if Instant::now() >= deadline {
            child.kill().expect("stop hung bl apps contract");
            child.wait().expect("reap hung bl apps contract");
            panic!("piped bl apps contract did not fail promptly without a session");
        }
        thread::sleep(Duration::from_millis(10));
    };
    let mut stdout = String::new();
    ...
    assert_eq!(status.code(), Some(3), "stderr was: {stderr}");
    assert!(stdout.is_empty(), "stdout was: {stdout}");
    assert!(stderr.contains("BuilderLab CLI auth is required"));
    assert!(!stderr.contains("Opening BuilderLab auth login"));
...
// ---------------------------------------------------------------------------
// bl tools passthrough

#[test]
fn bl_tools_help_surfaces_schema_derived_flags() {
    ...
    assert!(output.status.success(), "stderr was: {stderr}");
    assert!(stdout.contains("Usage: bl tools utils calculate"));
    ...
    assert_eq!(requests.len(), 1);
    assert_eq!(requests[0].path, BL_TOOLS_LIST_TOOLS_PATH);
    assert_eq!(requests[0].body["extension_name"], json!("utils"));
}

#[test]
fn bl_tools_forwards_stored_session_credential_to_kgoose_calls() {
    assert_bl_tools_session_credential(
        "bl-tools-session",
        "default",
        "stored-bl-tools-session",
        |_temp, _command| {},
    );
}

fn assert_bl_tools_session_credential(
    temp_name: &str,
    storage_profile: &str,
    ...
    let mut command = server.bl_tools_command();
    ...
    assert_eq!(requests[0].path, BL_TOOLS_LIST_TOOLS_PATH);
    assert_eq!(requests[1].path, BL_TOOLS_CALL_TOOL_PATH);
    fs::remove_dir_all(temp).expect("remove temp dir");
}

#[test]
fn bl_tools_resolves_session_credential_from_current_profile() {
    assert_bl_tools_session_credential(
        "bl-tools-current-profile-session",
        "local",
        ...
        |temp, command| {
            ...
            .expect("write skills config");
            command.env("BL_HOME", &bl_home);
        },
    );
}

#[test]
fn bl_tools_env_profile_does_not_require_readable_skills_config() {
    assert_bl_tools_session_credential(
        "bl-tools-env-profile-session",
        "env-profile",
        ...
        |temp, command| {
            ...
            command
                .env("BL_SKILLS_CONFIG", &malformed_config)
                .env("BL_SKILLS_PROFILE", "env-profile");
        },
    );
}

#[test]
fn bl_tools_malformed_skills_config_falls_back_to_default_profile() {
    assert_bl_tools_session_credential(
        "bl-tools-default-profile-session",
        "default",
        "stored-default-profile-session",
        |temp, command| {
            let malformed_config = temp.join("malformed-skills.yaml");
            fs::write(&malformed_config, "current_profile: [").expect("write malformed config");
            command.env("BL_SKILLS_CONFIG", &malformed_config);
        },
    );
}
...
#[cfg(unix)]
#[test]
fn bl_tools_root_metadata_commands_do_not_read_auth_storage() {
    ...
    for args in [
        ...
        let output = server
            ...
            .args(args)
            .output()
            .expect("run bl tools metadata command");
        let requests = server.finish();
        let (_stdout, stderr) = output_text(&output);
...
#[test]
fn bl_tools_describe_commands_uses_static_catalog_without_network() {
    let server = MockServer::start(vec![]);
    let catalog_path = write_extensions_catalog(
        "bl-tools-describe-commands",
        r#"
- name: secret
...
"#,
    ...
    let output = server
        ...
        .arg("--describe-commands")
        .output()
        .expect("run bl tools describe-commands");
    let requests = server.finish();
    let (stdout, stderr) = output_text(&output);
